...
export async function getDeviceDetails(req: any) {
  ...
  if (platformHeader && osHeader && osHeader !== 'web') {
     ...
     // CRITICAL FALLBACK: If Capacitor Device plugin failed (e.g. npx cap sync was missed),
     // X-Attendx-Hardware will literally be "Native Device".
     // We can still extract the exact hardware model from the Android WebView User-Agent!
     if (!finalDeviceString || finalDeviceString === 'Native Device') {
        const uaModel = result.device.model;
        const uaVendor = result.device.vendor;
        if (uaModel) {
           const marketName = getModelName(uaModel);
           if (marketName && marketName !== uaModel) {
              finalDeviceString = marketName;
           } else {
              finalDeviceString = uaVendor ? `${uaVendor} ${uaModel}` : uaModel;
           }
        }
     }
