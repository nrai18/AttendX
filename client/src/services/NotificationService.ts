...
export class NotificationService {
  ...
  static async init() {
    ...
    try {
      ...
      const stuckMute = delivered.notifications.find(n => Number(n.id) === 8888);
      if (stuckMute) {
        await unmutePhone();
        // Capacitor v6+ uses removeDeliveredNotifications, older versions used something else.
        // If it throws, we can safely ignore.
        try {
          // @ts-ignore
          if (LocalNotifications.removeDeliveredNotifications) await LocalNotifications.removeDeliveredNotifications({ notifications: [stuckMute] });
        } catch(e){}
      }
    } catch(e) {
    ...
    try {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        for (const e of dayEvents) {
          ...
          } else if (typeLower === 'exam' || (lowerTitle.includes('midsem') || lowerTitle.includes('endsem') || lowerTitle.includes('exam'))) {
            isClassOff = true;
            specialTitle = e.title;
  ...
  static async scheduleAcademicUpdates(frequency: { type: string, subValue?: string }) {
    ...
    try {
      ...
      if (frequency.type === 'Daily') {
        ...
        let firstNotifyDate = setMinutes(setHours(nextTargetDay, summaryHour), summaryMinute);
        if (firstNotifyDate.getTime() <= Date.now()) {
           firstNotifyDate = addDays(firstNotifyDate, 7);
        }

        for (let i = 0; i < 4; i++) {
           let notifyDate = addDays(firstNotifyDate, i * 7);
           
           let dynamicText = "📅 Weekly Review\n\nTake a look back at your attendance performance this week and plan ahead for the upcoming week.";
           ...
           const weekEvents = allEvents.filter(e => {
             const eDate = startOfDay(new Date(e.date));
             return eDate.getTime() >= startOfWeekDay.getTime() && eDate.getTime() <= endOfWeekDay.getTime();
           });

           if (weekEvents.some(e => (e.eventType || "").toLowerCase() === "holiday" || (e.title || "").toLowerCase().includes("holiday"))) {
        ...
        let firstNotifyMonth = today.getMonth();
        let initialNotifyDate = setMinutes(setHours(new Date(today.getFullYear(), firstNotifyMonth, targetDate), summaryHour), summaryMinute);
        if (initialNotifyDate.getTime() <= Date.now()) {
            firstNotifyMonth++;
        }

        for (let i = 0; i < 3; i++) {
           let notifyDate = setMinutes(setHours(new Date(today.getFullYear(), firstNotifyMonth + i, targetDate), summaryHour), summaryMinute);
