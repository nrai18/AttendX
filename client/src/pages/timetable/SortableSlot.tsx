...
export const SortableSlot: React.FC<SortableSlotProps> = ({ slot, onEdit, onDelete, isDesktop }) => {
  ...
  if (isDesktop) {
    return (
      <div 
        ref={setNodeRef}
        style={style}
        className="relative group p-3 rounded-xl border border-border bg-card/80 hover:border-primary/50 hover:shadow-md transition-all duration-300"
        {...attributes}
        {...listeners}
      >
        {/* Color Accent */}
        ...
        <div className="pl-2">
          <div className="flex justify-between items-start mb-1">
            <span className="text-xs font-bold text-foreground/90">{slot.startTime}</span>
  ...
  return (
    <div 
      ref={setNodeRef}
      style={style}
      className="flex items-center justify-between p-4 bg-card border border-border rounded-2xl group hover:border-border/80 transition-colors shadow-sm"
    >
      <div className="flex items-center gap-2 flex-1 min-w-0">
        <div 
          {...attributes}
          {...listeners}
          className="p-1 -ml-2 text-muted-foreground hover:text-foreground cursor-grab active:cursor-grabbing touch-none"
        >
        ...
        <div className="flex-1 min-w-0 ml-2">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1.5 text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
            <span className="bg-muted px-2 py-0.5 rounded-md text-foreground/90 font-semibold shadow-sm">{slot.startTime} - {slot.endTime}</span>
      ...
      <div className="flex items-center gap-1 ml-3">
        <button
          onClick={() => onEdit(slot)}
          className="p-3 text-muted-foreground hover:text-blue-500 bg-muted hover:bg-muted/80 rounded-xl transition-colors"
        >
          <Edit2 className="w-4 h-4" />
        </button>
        <button
          onClick={() => onDelete(slot.id)}
          className="p-3 text-muted-foreground hover:text-rose-500 bg-muted hover:bg-muted/80 rounded-xl transition-colors"
        >
